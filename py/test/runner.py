# Fastapi SDK test runner

from __future__ import annotations
import os
import json

from fastapi_sdk.utility.voxgig_struct import voxgig_struct as vs
...
class FastapiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FastapiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FastapiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FastapiTestRunner.getenv("FASTAPI_TEST_LIVE")
        override = FastapiTestRunner.getenv("FASTAPI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FastapiTestRunner.getenv(key)
        ...
        explain = FastapiTestRunner.getenv("FASTAPI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FASTAPI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FastapiTestRunner._test_control is not None:
            return FastapiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FastapiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FastapiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FastapiTestRunner.load_test_control()
...
def load_env_local():
    FastapiTestRunner.load_env_local()


def env_override(m):
    return FastapiTestRunner.env_override(m)


def entity_data(v):
    return FastapiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FastapiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FastapiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FastapiTestRunner.load_test_control()


def live_client_options():
    return FastapiTestRunner.live_client_options()


def live_delay_ms():
    return FastapiTestRunner.live_delay_ms()
