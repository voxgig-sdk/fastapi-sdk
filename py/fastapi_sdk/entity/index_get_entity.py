...
from fastapi_sdk.fastapi_types import (
    IndexGet,
    IndexGetLoadMatch,
)
...
class IndexGetEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return IndexGetEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> IndexGet:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.IndexGet().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
