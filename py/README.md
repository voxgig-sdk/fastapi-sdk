...
The Python SDK for the Fastapi API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.IndexGet(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Fastapi(None).load(
    {"id": "test01"}, None
)
