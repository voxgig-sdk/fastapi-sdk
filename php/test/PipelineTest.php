...
{
    private static function utility(): FastapiUtility
    {
        return new FastapiUtility();
    }
    ...
    private static function ctx(array $over = []): FastapiContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FastapiContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FastapiOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FastapiError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FastapiOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FastapiMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FastapiOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FastapiMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FastapiOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FastapiMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FastapiSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FastapiMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FastapiResponse([]),
            'result' => new FastapiResult([]),
        ]);
        [, $err] = FastapiMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FastapiSpec([]),
            'response' => null,
            'result' => new FastapiResult([]),
        ]);
        [, $err] = FastapiMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FastapiSpec(['step' => 's']),
            'response' => new FastapiResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FastapiResult([]),
        ]);
        [, $err] = FastapiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FastapiSpec(['step' => 's']),
            'response' => new FastapiResponse(self::resp(200, ['v' => 1])),
            'result' => new FastapiResult([]),
        ]);
        [, $err] = FastapiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FastapiSpec(['step' => 's']),
            'response' => new FastapiResponse(self::resp(200, ['v' => 2])),
            'result' => new FastapiResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FastapiMakeResponse::call($ctx);
    ...
    {
        $preset = new FastapiResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FastapiSpec([]),
            'response' => new FastapiResponse([]),
            'result' => new FastapiResult([]),
        ...
        [$got, $err] = FastapiMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FastapiResult([])]);
        [, $err] = FastapiMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FastapiSpec([]), 'result' => null]);
        [, $err] = FastapiMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FastapiSpec(['step' => 's']),
            'result' => new FastapiResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FastapiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FastapiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FastapiSpec(['step' => 's']),
            'result' => new FastapiResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FastapiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FastapiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FastapiResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FastapiSpec([]),
            'result' => new FastapiResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FastapiMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FastapiMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FastapiError('boom', 'boom');
        $utility->fetcher = function (FastapiContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FastapiSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FastapiMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FastapiContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FastapiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FastapiResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FastapiContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FastapiSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FastapiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FastapiResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FastapiSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FastapiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FastapiMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FastapiMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FastapiResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FastapiDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FastapiResult(['ok' => false])]);
        $this->expectException(FastapiError::class);
        FastapiDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FastapiResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FastapiMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FastapiMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FastapiBaseFeature();
        $b = new FastapiBaseFeature();
        FastapiFeatureAdd::call($ctx, $a);
        FastapiFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FastapiBaseFeature
    {
        $f = new FastapiBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FastapiFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FastapiContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FastapiSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FastapiPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FastapiResponse(['headers' => null]),
            'result' => new FastapiResult([]),
        ]);
        FastapiResultHeaders::call($ctx);
