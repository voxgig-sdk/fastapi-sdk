...
// Fastapi SDK primary utility test
//
...
//   FastapiOmni::unwrap, which throws the err so omni can match it
...
//   builds the typed context with FastapiOmni::ctx_from_map, runs the
//   utility, and writes the observable state back with
//   FastapiOmni::sync_ctx BEFORE throwing any error - that map is
...
class PrimaryUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = FastapiOmni::makeRunner(
                self::TEST_JSON_FILE, FastapiSDK::test(null, null));
    ...
    private static function client(): FastapiSDK
    {
    ...
    public function test_done_basic(): void
    {
        ...
        $this->runsection('done', function (array $ctxmap) use ($client, $utility) {
            $ctx = FastapiOmni::ctx_from_map($ctxmap, $client, $utility);
    ...
    public function test_make_error_basic(): void
    {
        ...
        $this->runsection('makeError', function ($ctxmap = null, $errmap = null) use ($client, $utility) {
            $ctx = FastapiOmni::ctx_from_map(
                is_array($ctxmap) ? $ctxmap : [], $client, $utility);
            $err = FastapiOmni::err_from_map(is_array($errmap) ? $errmap : null);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new FastapiBaseFeature();
    ...
    {
        $hook_client = FastapiSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends FastapiBaseFeature {
            public $hook_fn = null;
            public function TestHook(FastapiContext $ctx): void
            {
    ...
    {
        $init_client = FastapiSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends FastapiBaseFeature {
            public $init_fn = null;
    ...
    public function test_make_request_basic(): void
    {
        ...
        $this->runsection('makeRequest', function (array $ctxmap) use ($client, $utility) {
            ...
            FastapiOmni::sync_ctx($ctxmap, $ctx);
            if ($err !== null) {
                throw $err;
            }
            return null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new FastapiSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_url_basic(): void
    {
        ...
        $this->runsection('makeUrl', function (array $ctxmap) use ($client, $utility) {
            ...
            return FastapiOmni::unwrap(($utility->make_url)($ctx));
        });
    }
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = FastapiSDK::test(null, is_array($setup) ? $setup : null);
        ...
        $this->runsection('prepareAuth', function (array $ctxmap) use ($auth_client, $auth_utility, $utility) {
            $ctx = FastapiOmni::ctx_from_map($ctxmap, $auth_client, $auth_utility);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runsection('resultBasic', function (array $ctxmap) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof FastapiError)
