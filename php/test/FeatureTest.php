...
class FtCtrl extends FastapiControl
{
...
class FtClient
{
    ...
    private FastapiUtility $utility;

    public function __construct(FastapiUtility $utility, array $options)
    {
    ...
    public function get_utility(): FastapiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FastapiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FastapiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FastapiConfig::make_feature($name);
    ...
    private static function buildUrl(FastapiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FastapiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FastapiResponse($fetched);
        }
    ...
    private function fail(FastapiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FastapiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
