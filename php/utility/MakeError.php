...
// Fastapi SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class FastapiMakeError
{
    public static function call(?FastapiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FastapiContext([], null);
        }
        $op = $ctx->op ?? new FastapiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FastapiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FastapiError) ? $err->msg : (string)$err;
        $msg = "FastapiSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new FastapiError('', $msg, $ctx);
