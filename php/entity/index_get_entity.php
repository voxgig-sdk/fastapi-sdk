...
class IndexGetEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new IndexGetEntity($this->_client, $opts);
    }

    /**
     * @param IndexGet|array $args IndexGet data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = FastapiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return IndexGet|array The current IndexGet data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of IndexGet fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = FastapiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of IndexGet fields).
     */
    ...
    /**
     * Load a single IndexGet.
     *
     * @param IndexGetLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed IndexGetLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return IndexGet|array The loaded IndexGet as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FastapiHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
