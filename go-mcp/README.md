...
[MCP](https://modelcontextprotocol.io) server exposing the Fastapi SDK as
two agent tools — `fastapi_list` and `fastapi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fastapi-mcp)
make build
...
claude mcp add --scope user fastapi \
  -- /absolute/path/to/fastapi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./fastapi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// fastapi_list: first page of records
{ "entity": "index_get" }
{ "entity": "index_get", "query": { } }

// fastapi_load: one record by id
{ "entity": "index_get", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fastapi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fastapi \
     -- "$PWD"/dist/*/fastapi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fastapi_list` and `fastapi_load` tools now appear
   in new sessions. Ask the agent to *"list index_get using fastapi"*
   and it calls `fastapi_list` with `{"entity":"index_get"}`.
...
```sh
export FASTAPI_APIKEY=sk_live_xxx            # API key
export FASTAPI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `fastapi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fastapi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FASTAPI_APIKEY` | API key sent with every request. |
| `FASTAPI_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

index_get | iprank | json | robot | simple | table

### Smoke test via HTTP (raw JSON-RPC)

```sh
./fastapi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"fastapi_load","arguments":{"entity":"index_get","query":{"id":1}}}}'
```
