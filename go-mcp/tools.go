...
type Args struct {
	Entity string         `json:"entity" jsonschema:"index_get | iprank | json | robot | simple | table"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.FastapiSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "fastapi_list",
		Description: "List records from Fastapi. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "fastapi_load",
		Description: "Load a single record from Fastapi. " +
...
func runOp(client *sdk.FastapiSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.FastapiSDK, name string) (sdk.FastapiEntity, error) {
	switch strings.ToLower(name) {
	case "index_get":
		return client.IndexGet(nil), nil
	case "iprank":
		return client.Iprank(nil), nil
	case "json":
		return client.Json(nil), nil
	case "robot":
		return client.Robot(nil), nil
	case "simple":
