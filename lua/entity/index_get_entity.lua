...
local IndexGetEntity = {}
IndexGetEntity.__index = IndexGetEntity


function IndexGetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IndexGetEntity)
  self._name = "index_get"
  self._client = client
...
function IndexGetEntity:get_name()
  return self._name
end


function IndexGetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IndexGetEntity.new(self._client, opts)
end
...
function IndexGetEntity:mark_deleted()
  self._deleted = true
end


function IndexGetEntity:deleted()
  return true == self._deleted
end


function IndexGetEntity:data_set(args)
  if args ~= nil then
...
function IndexGetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IndexGetEntity:match_set(args)
  if args ~= nil then
...
function IndexGetEntity:match_get()
...
function IndexGetEntity:stream(action, args, callopts)
...
function IndexGetEntity:_run_op(ctx, post_done)
  local utility = self._utility
