...
local IprankEntity = {}
IprankEntity.__index = IprankEntity


function IprankEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IprankEntity)
  self._name = "iprank"
  self._client = client
...
function IprankEntity:get_name()
  return self._name
end


function IprankEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IprankEntity.new(self._client, opts)
end
...
function IprankEntity:mark_deleted()
  self._deleted = true
end


function IprankEntity:deleted()
  return true == self._deleted
end


function IprankEntity:data_set(args)
  if args ~= nil then
...
function IprankEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IprankEntity:match_set(args)
  if args ~= nil then
...
function IprankEntity:match_get()
...
function IprankEntity:stream(action, args, callopts)
...
function IprankEntity:_run_op(ctx, post_done)
  local utility = self._utility
