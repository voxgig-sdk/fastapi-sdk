...
The Lua SDK for the Fastapi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:IndexGet(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Fastapi(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── fastapi_sdk.lua    -- Main SDK module
...
The main module (`fastapi_sdk`) exports the SDK constructor
