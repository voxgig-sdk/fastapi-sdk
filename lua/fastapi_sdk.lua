...
FastapiSDK._make_feature = _make_feature


function FastapiSDK.new(options)
  local self = setmetatable({}, FastapiSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FastapiSDK:options_map()
...
function FastapiSDK:get_utility()
  return Utility.copy(self._utility)
end


function FastapiSDK:get_root_ctx()
  return self._rootctx
end


function FastapiSDK:prepare(fetchargs)
...
function FastapiSDK:direct(fetchargs)
...
-- by passing it.
function FastapiSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FastapiSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FastapiSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:IndexGet():list() / client:IndexGet():load({ id = ... })
...
function FastapiSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FastapiSDK.new(sdkopts)
  sdk.mode = "test"
