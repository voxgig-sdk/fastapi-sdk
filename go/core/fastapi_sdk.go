...
func (sdk *FastapiSDK) OptionsMap() map[string]any {
...
func (sdk *FastapiSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *FastapiSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *FastapiSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *FastapiSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *FastapiSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *FastapiSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("FastapiSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *FastapiSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *FastapiSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("FastapiSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
