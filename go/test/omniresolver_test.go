...
import (
	...
	omni "github.com/voxgig-sdk/fastapi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FastapiError); is {
				out := map[string]any{
					"name":    "FastapiError",
...
func omniCtx(arg any, client *sdk.FastapiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FastapiSDK); is {
			client = live
