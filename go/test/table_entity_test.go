...
func TestTableEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FASTAPI_TEST_TABLE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		tableRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.table", setup.data)))
		var tableRef01Data map[string]any
		if len(tableRef01DataRaw) > 0 {
			tableRef01Data = core.ToMapAny(tableRef01DataRaw[0][1])
		}
		...
		tableRef01MatchDt0 := map[string]any{}
		tableRef01DataDt0Loaded, err := tableRef01Ent.Load(tableRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		if tableRef01DataDt0Loaded == nil {
...
func tableBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FASTAPI_TEST_TABLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FASTAPI_TEST_TABLE_ENTID"])
