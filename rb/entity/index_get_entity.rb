...
class IndexGetEntity
  ...
  def make
    opts = @_entopts.dup
    IndexGetEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FastapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [IndexGet, Hash] the current IndexGet data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FastapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of IndexGet fields)
  def match_get
  ...
  #
  # @param reqmatch [IndexGetLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.IndexGet.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [IndexGet, Hash] the loaded IndexGet; raises FastapiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FastapiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
