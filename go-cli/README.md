...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fastapi-cli)
make build
...
FASTAPI_BASE=https://api.example.com ./fastapi-cli load 1 index_get

# 6. No arguments -> interactive REPL
./fastapi-cli
fastapi> load 1 index_get
fastapi> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fastapi-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/fastapi-cli load 1 index_get
   ```
...
```sh
./fastapi-cli load 1 index_get          # scalar shorthand for {id:1}
./fastapi-cli load '{id:1}' index_get     # explicit match map
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fastapi-cli`. |
