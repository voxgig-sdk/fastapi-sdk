// Command Fastapi-cli is a CLI + REPL for the Fastapi SDK,
...
const prompt = "fastapi"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "index_get iprank json robot simple table"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FASTAPI_APIKEY (api key), FASTAPI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FASTAPI_APIKEY carries the API key and
	// FASTAPI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FASTAPI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FASTAPI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFastapiSDK(opts)
