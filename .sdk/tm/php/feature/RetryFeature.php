...
class FastapiRetryFeature extends FastapiBaseFeature
{
    ...
    public function init(FastapiContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FastapiContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FastapiContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FastapiContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
