...
module FastapiUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = FastapiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FastapiResponse.new(fetched)
    else
