...
describe('IndexGetDirect', async () => {
  ...
  test('direct-load-index_get', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-index_get', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FASTAPI_TEST_INDEX_GET_ENTID']
