...
//     (\`new FastapiSDK(...)\` / \`FastapiSDK.test(...)\` become
//     \`FastapiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"index_get":{"test01":{"id":"test01"}},"iprank":{"test01":{"id":"test01"}},"json":{"test01":{"id":"test01"}},"robot":{"test01":{"id":"test01"}},"simple":{"test01":{"id":"test01"}},"table":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FastapiSDK(...)\` and any
// \`FastapiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FastapiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FastapiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
